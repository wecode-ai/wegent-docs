# 🛠️ Developer Guide

English | [简体中文](../../zh/developer-guide/README.md)

Welcome to the Wegent Developer Guide! This guide will help you understand how to develop and extend Wegent.

---

## 📋 Table of Contents

### 🚀 Getting Started

- [Development Setup](./setup.md) - Configure your local development environment

### 🧪 Testing

- [Testing Guide](./testing.md) - Writing and running tests

### 🗄️ Database

- [Database Migrations](./database-migrations.md) - Managing database migrations

### 📱 Frontend Development

- [Responsive Development](./responsive-development.md) - Responsive design guidelines

### 📊 Observability

- [OpenTelemetry](./opentelemetry.md) - Tracing and observability setup

---

## 📖 Related Documentation

- [Core Concepts](../concepts/core-concepts.md) - Understand Wegent's core architecture
- [YAML Specification](../reference/yaml-specification.md) - CRD YAML specification reference
- [User Guide](../user-guide/README.md) - User guide

---

## 💬 Get Help

Need help?

- 📖 Check the [FAQ](../faq.md)
- 🐛 Submit a [GitHub Issue](https://github.com/AntGroup/Wegent/issues)
- 💬 Join community discussions
