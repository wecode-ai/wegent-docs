...
When Executor prepares a Git workspace, it disables interactive credential prompts and enforces both an overall timeout and Git HTTP low-speed limits. `WEGENT_GIT_CLONE_TIMEOUT_SECONDS` controls the overall timeout, defaults to 600 seconds, and is clamped to 1-3600 seconds. `WEGENT_GIT_HTTP_LOW_SPEED_LIMIT` and `WEGENT_GIT_HTTP_LOW_SPEED_TIME_SECONDS` default to 1024 bytes/second and 60 seconds. When the overall timeout expires, Executor terminates the entire Git process group, removes the incomplete workspace created by that clone, and returns a terminal task error. Existing workspaces must also pass a local `HEAD` validation so a `.git` directory left by an interrupted clone is not treated as a usable repository. The flow does not retry automatically.

Claude Code retains request MCP configurations per task: services omitted from a later turn remain available, while a matching service is replaced by its complete incoming configuration, removing stale URLs, headers, and transport settings. The executor stores the merged map in `.wework/runtime/claude-mcp-cache-<task_id>.json` and writes a separate `claude-mcp-<task_id>-<subtask_id>.json` for each turn. Tasks sharing a workspace keep separate caches. Executor-managed MCP credentials are injected afresh for each turn and are not stored in this cache.
...
Codex hook plugins receive `PostToolUse` from the live turn stream by default, so a file written by a subagent thread, or while the executor was not watching (offline, restarting, or with the plugin disabled), never reaches them. The executor therefore also offers a durable rollout feed: a plugin that declares `codex_rollout` in the `subscriptions` list of its `plugin.json` receives its `PostToolUse` hooks from an observer instead. The observer tails the rollouts under `sessions/` and `archived_sessions/` of the isolated Codex Home, delivers one file change per edited file, and resolves subagent threads through `parent_thread_id` up to the task thread, so their writes count towards the same task. Deliveries are keyed by "Codex call id + canonical file path": one edit mirrored by a parent and a subagent session is delivered once, while repeated edits of one file are delivered once per edit. Rollouts that predate the observer resume at end of file, and newly created ones start right after `session_meta`. Plugins that do not subscribe keep the live dispatch, and the observer does not scan while nothing subscribes. Pending deliveries commit together with the read cursor in SQLite and retry with backoff when the hook fails.
...
When the supervisor decides to continue with a correction, it must not treat the process-local Codex notification routing cache as the source of truth for execution configuration. The executor persists a stable execution request with the runtime task: it retains recoverable Agent, model routing, MCP, Skill, knowledge-scope, and permission configuration while clearing the prompt, history, message ID, Backend address, and temporary task/runtime/skill tokens, and recursively removes API keys, Authorization values, cookies, passwords, private keys, and header secrets. Continuations, model switches, and forks update or inherit this configuration. An automatic correction rebuilds its request from that profile and then injects current credentials from the active Backend connection, so an Executor restart or context compaction preserves the original route and capabilities without reusing expired credentials.
...
When a fork is created from a completed turn, a successful `runtime.tasks.fork_at_turn` response must include the canonical transcript produced by Codex at that `lastTurnId` boundary and persist the same snapshot in the new task's runtime handle. Wework seeds the target conversation cache and lifecycle store before opening the task; it must not first navigate to an empty pane and depend on another transcript RPC that can be queued behind background project synchronization. The snapshot must come from the new forked thread rather than copying the source pane's currently visible messages, which could leak turns created after the fork boundary.
