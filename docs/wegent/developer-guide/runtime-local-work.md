...
Rich references sent by Wework, such as skill or plugin tokens, may be normalized to plain text in the Codex provider transcript. To preserve the Codex transcript as the single message source, Wework persists only each reference token and target by `clientMessageId` in LocalTask `runtimeHandle.userMessagePresentations`; it does not store a second user-message body. After reading the provider transcript, the executor keeps provider `content` as the only body and adds only the reference ranges and targets that the transcript cannot express to the message with the same `clientMessageId`. Those non-overlapping additions are returned as `presentationReferences`; without a stable client message ID, the executor does not guess a match. This composition applies to running, successful, and failed tasks, and Wework combines the body and presentation references at the rendering boundary so reopening a completed task keeps skill and plugin references rendered as tokens instead of degrading to raw `$plugin:skill` or `@Plugin` text.
