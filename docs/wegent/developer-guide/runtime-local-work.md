...
Wework must not periodically read transcripts for running tasks to detect a missed terminal event. The normal lifecycle consumes only the stream. The resident coordinator settles the shared lifecycle store directly from a terminal event's `deviceId + taskId`, so completion does not depend on a mounted task pane and cannot be overwritten by a task-list read that arrives while the Executor and provider are still finishing the same turn. Only explicit stale-projection signals such as `executor.event_lagged` and runtime transport replacement trigger one `runtime.tasks.list` reconciliation. Concurrent anomaly signals share one in-flight request, and signals received while it is in flight coalesce into at most one serial trailing reconciliation. Reconciliation uses the executor's persisted `running`, `status`, `completedAt`, `threadStatus`, and `turnStatus` fields directly; it must not derive task completion from transcripts, turn items, or rollout JSONL.
...
The live stream and Provider snapshot can represent the same assistant text differently within one turn. For example, realtime events may produce `block:text` while a restored legacy conversation returns `assistant_text`. When merging a turn, the frontend must treat items with identical content and these complementary types as one message, replacing the live representation with the snapshot representation. Merging only by item id renders the same streamed text twice after following up in an older conversation. Same-type items with identical text remain distinct by id so genuinely repeated model output is preserved. `runtimeConversationTurns` unit tests and the real Electron `streaming-text` E2E cover this boundary.
...
When the archived LocalTask uses an Executor-managed Git worktree, Wework calls the device-scoped `runtime.worktrees.delete` with the LocalTask's own `workspacePath` after archive succeeds. Even when the sidebar groups the task under the Project root, that grouping path must not become the deletion target. The Executor writes tracked, staged, unstaged, and non-ignored untracked files to a hidden `refs/wegent/worktree-snapshots/*` reference before Git removes the worktree, then removes the now-empty outer `<worktreeId>` directory. A snapshot failure must preserve the directory and return an error; uncommitted changes must not be discarded. When a task is unarchived or receives another message after its directory was cleaned, the Executor restores the original path from the snapshot reference. This lifecycle applies only to runtime LocalTask worktrees and does not mutate the Project root workspace. A LocalTask created by forking a message may share its source task's managed worktree; deleting one archived task must clean only that task's attachments while another LocalTask still references the path.
...
Wework manages worktrees through device-scoped `runtime.worktrees.settings.get/update` and `runtime.worktrees.prepare/list/delete/restore/prune` RPCs. Creation targets are fixed to `<resolvedRoot>/<worktreeId>/<repositoryName>`. Lists are grouped by repository and include linked LocalTasks. The creation ID owns the path, snapshot, and lifecycle metadata, while the execution lease records the LocalTask that is actually running. This lets a fork continue in an idle source worktree while preventing two tasks from writing to the same worktree concurrently; after an Executor restart, the lease task is the task marked as interrupted. Deletion archives linked tasks and captures a snapshot first. Archived-file cleanup validates paths through the Worktree Manager's current root and `knownRoots` instead of relying on fixed legacy directory names; scanning known roots also removes empty `<worktreeId>` directories left by older versions. Automatic cleanup runs after creation and settings updates and only removes least-recently-used worktrees above the retention limit when they are explicitly linked exclusively to archived tasks. Worktrees without a task record in the current Executor are never auto-cleaned. Continuing a cleaned task restores it on demand. Isolated Executor instances derive their default worktree directory from their own `WEGENT_EXECUTOR_HOME`, preventing test or development instances from managing production worktrees.
...
One Codex user turn can contain an attachment wrapper, application context, and a user-message event at the same time. The executor must first extract the user-visible request text, then use it to merge duplicate messages from that turn while retaining `clientUserMessageId` and attachment metadata. Turn-navigation IDs prefer `clientUserMessageId`; previews for loaded turns must be derived from the same visible text and must not expose runtime-injected content such as `# Files mentioned by the user` or `<application_context>`.

Rich references sent by Wework, such as skill or plugin tokens, may be normalized to plain text in the Codex provider transcript. To preserve the Codex transcript as the single message source, Wework persists only each reference token and target by `clientUserMessageId` in LocalTask `runtimeHandle.userMessagePresentations`; it does not store a second user-message body. After reading the provider transcript, the executor keeps provider `content` as the only body and adds only the reference ranges and targets that the transcript cannot express to the message with the same `clientUserMessageId`. Those non-overlapping additions are returned as `presentationReferences`; without a stable client user message ID, the executor does not guess a match. This composition applies to running, successful, and failed tasks, and Wework combines the body and presentation references at the rendering boundary so reopening a completed task keeps skill and plugin references rendered as tokens instead of degrading to raw `$plugin:skill` or `@Plugin` text.
