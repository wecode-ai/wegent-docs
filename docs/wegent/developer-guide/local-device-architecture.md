...
A conversation can switch models between turns. Wework sends the selected model and provider configuration with every continuation. Before resuming an idle Codex thread, the executor releases its current app-server subscription so `thread/resume` can apply the new provider overrides; the resume operation then subscribes again. Without this release, Codex retains the loaded thread's previous provider even when the model name in `turn/start` changes, so the real request can still reach the old custom-model upstream. The executor also writes the turn's `modelSelection` back to the task summary so the model shown after a refresh matches the model that handled the request. Guidance sent while a turn is running remains part of that turn and does not switch models; the new model applies only to a normal new turn or one created by interrupt-and-send.
