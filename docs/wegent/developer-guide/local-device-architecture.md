...
The active-task set owned by the current executor process is the only authority for `running`. Codex thread lists, transcripts, persisted task summaries, and Wework reminder state provide content or historical metadata only; they must not independently infer that a task is still executing. The task list, detail pane, system tray, and sleep-inhibition logic must consume the same executor-provided `running` value. After an executor restart, a thread absent from the new process's active set is idle even if Codex metadata briefly remains `active` or `inProgress`.
...
Frontend execution state is memory-only and is never written to a local file or browser storage. The optimistic `starting` state created when a user sends a message is owned by the same state machine and converges when the executor explicitly reports `running=true` or `running=false`. During automatic continuation of an active Goal, the task remains visibly running between turns while the executor keeps it in the current process's active set, but the turn state is `idle`, so Wework shows no thinking indicator and creates no unread marker. After Wework or the executor restarts, `running=false` from the new process makes the task idle even if the Goal remains `active`, and execution does not resume automatically. Only unread completion reminders are persisted, and they can never be used to infer execution state.
...
A conversation can switch models between turns within the same provider category. Wework sends the selected model and provider configuration with every continuation. The executor assigns one stable local model-proxy URL to each task and atomically updates that task's upstream configuration at the start of every turn. The Codex thread always resumes through the same task proxy, so switching models within the category does not require restarting the app-server, forking the thread, or selecting another proxy from a possibly stale model name in the Codex request body. After the root thread is created, the proxy binds its thread ID and accepts requests only from that thread or its child threads. The upstream and model supplied by the executor for the current turn are the sole routing authority.

Official Codex creates the thread with the OpenAI provider identity, while third-party models run through the Wework router provider. Codex app-server does not allow a resumed thread to change providers, so the frontend disables cross-category models in an existing conversation: an official Codex conversation cannot switch to a third-party model, and a third-party conversation cannot switch to official Codex. A new conversation can still start with any available model. To continue context across categories, the user can start a new conversation and use `@` to reference the source conversation.

After an explicit failure, “switch model and retry” starts one new turn. It preserves the original task and thread context, limits selection to the same provider category, and sends exactly one request to the newly selected upstream. The executor also writes the turn's `modelSelection` back to the task summary so the model shown after a refresh matches the model that handled the request. Guidance sent while a turn is running remains part of that turn and does not switch models; the new model applies only to a normal new turn or one created by interrupt-and-send.
