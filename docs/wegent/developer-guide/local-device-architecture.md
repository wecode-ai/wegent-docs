...
A conversation can switch models between turns. Wework sends the selected model and provider configuration with every continuation. Before resuming an idle Codex thread, the executor waits until its current app-server subscription is actually released so `thread/resume` can apply the new provider overrides; the resume operation then subscribes again. Without waiting for the release response, Codex can retain the loaded thread's previous provider even when the model name in `turn/start` changes, so the real request can still reach the old custom-model upstream. If a loaded thread still caches its old local router URL, the local model proxy also selects the active registration for the requested model within the same gateway URL and credential scope. This keeps model switching on `resume`, preserves the original conversation context without forking the thread, and prevents credential reuse across users or providers. The executor also writes the turn's `modelSelection` back to the task summary so the model shown after a refresh matches the model that handled the request. Guidance sent while a turn is running remains part of that turn and does not switch models; the new model applies only to a normal new turn or one created by interrupt-and-send.
