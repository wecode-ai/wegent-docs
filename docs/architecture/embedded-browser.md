...
flowchart LR
    ...
    PANEL -->|user navigation intent| LOADING[tab loading indicator]
...
sequenceDiagram
    ...
    Note over R,W: For address-bar and reload actions, React enters loading before invoking the native command
    W->>H: GET URL
    H-->>W: response
