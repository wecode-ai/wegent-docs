...
flowchart LR
    ...
    RUNTIME[Single-file Wework-managed DSH runtime archive] --> EXTRACT[(Content-addressed extraction)]
    EXTRACT --> INSTANCE
...
| Runtime archive, content fingerprint, and build preparation         | `wework/scripts/prepare-deepseek-harness-runtime.mjs`, `wework/src-tauri/tauri.conf.json`                   |
