...
The macOS tray must use the default menu bar item identity generated by
Electron. Do not pass a custom GUID to the `Tray` constructor. Menu bar
managers such as iBar persist visibility rules by the default item name and
application identifier; introducing a GUID later makes the same application
look like a new icon and invalidates the user's existing **Always Show**
setting. Debug instances remain isolated from the release app and other
worktrees through their distinct `WEWORK_APP_IDENTIFIER` values.
