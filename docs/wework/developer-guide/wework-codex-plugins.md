...
The Applications page reads its lists through `GET /api/sites`. Sites and Mini Programs share this endpoint and pass `app_type=web` and `app_type=miniapp`, respectively. Omitting the parameter defaults to Sites for backward compatibility. Platform uses the trusted user identity to return projects that the user owns or collaborates on. The response identifies the owner and current user's `owner` or `collaborator` role with `owner_username` and `access_role`, while `app_type` discriminates the fields for each application type. The page also calls `GET /api/sites/app-types` to discover the types enabled by the current Backend, their display order, and capabilities such as `create`, `publish`, `edit`, `delete`, `open_experience`, `configure_environment`, and `manage_access`. Wework only shows types that are both enabled by the server and represented by a local Definition, and hides operations that are not supported by the advertised capabilities. URLs in the Sites list always open in the system browser, regardless of the Wework embedded-browser preference.
...
Backend remains the authenticated management proxy for Sites; Wework never calls Platform directly. Collaborator management uses `GET/POST /api/sites/{siteid}/collaborators` and `DELETE /api/sites/{siteid}/collaborators/{subject}`. Only owners see this management entry in the UI, and add requests require an `Idempotency-Key`. Internal Site access policies use `GET/PUT /api/sites/{siteid}/access`; both owners and collaborators can read and update them. The supported audiences are anonymous access, signed-in users, project members only, and project members plus specified users, and writes require an `Idempotency-Key`. Environment configuration uses `GET/PATCH /api/sites/{siteid}/environment-variables`, with single-variable `PUT/DELETE /api/sites/{siteid}/environment-variables/{key}` endpoints also available. Writes carry an idempotency key and an optional `expected_revision_id`; clients reload after a conflict. Backend forwards the authenticated user's `X-Wegent-Username` to Platform and marks environment responses `Cache-Control: no-store`. Incoming and outgoing environment HTTP bodies must not be captured in request logs or telemetry. Secret responses expose configured metadata only and never return the value.
