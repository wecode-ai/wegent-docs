...
Backend remains the authenticated management proxy for Sites; Wework never calls Platform directly. Collaborator management uses `GET/POST /api/sites/{siteid}/collaborators` and `DELETE /api/sites/{siteid}/collaborators/{subject}`. Only owners see this management entry in the UI, and add requests require an `Idempotency-Key`. Environment configuration uses `GET/PATCH /api/sites/{siteid}/environment-variables`, with single-variable `PUT/DELETE /api/sites/{siteid}/environment-variables/{key}` endpoints also available. Writes carry an idempotency key and an optional `expected_revision_id`; clients reload after a conflict. Backend forwards the authenticated user's `X-Wegent-Username` to Platform and marks environment responses `Cache-Control: no-store`. Incoming and outgoing environment HTTP bodies must not be captured in request logs or telemetry. Secret responses expose configured metadata only and never return the value.
