...
`wegent-sites` is maintained in a separate plugin repository. Before a Backend image build, `pnpm prepare:builtin-plugins` copies the external plugin into the ignored `backend/init_data/plugins/wegent-sites` staging directory. The standard `build_image.sh` and `build_image_mac.sh` scripts run this step automatically. Official image workflows download the plugin from a configured archive URL, verify its pinned SHA-256, and run the same staging operation. Download, verification, or staging failures stop the image build, and every Backend process validates the required plugin directory and manifest on startup. Backend then idempotently publishes the plugin as a public, featured Wegent cloud marketplace item owned by the system `user_id=0`.

The Sites create action calls `POST /api/plugins/builtin/wegent-sites/ensure-installed` with the target `device_id`. The endpoint only installs public plugins published by the system owner. Repeated calls reuse and re-enable the existing installation record, merge only the installed `wegent-sites` plugin into that device, and verify the installation ID and `synced` acknowledgement. Failures from unrelated historical skills or plugins do not block Sites chat creation; a missing or offline target, or a failure to synchronize Sites itself, still prevents chat creation. After confirmation, the frontend opens a new task with the stable `plugin://wegent-sites@wegent` reference; clicking that mention loads the cloud plugin detail.
