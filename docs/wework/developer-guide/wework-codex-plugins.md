...
Normal uninstall removes the account install intent, device desired state, Codex app-server installed record, and connector login state according to the plugin auth policy. It does not proactively delete reusable package caches under Codex or Claude `plugins/cache`; the runtime owns those caches for reuse and garbage collection. Disk cleanup should be implemented as a separate cache purge or GC flow that deletes only versions no longer referenced by any installation record.
