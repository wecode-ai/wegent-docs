...
The picker's local and cloud catalogs are independent. The local catalog depends on the local Executor state and on the desktop preferences, so when either prerequisite fails it must degrade to local custom models only: it must not fail the whole list and must not keep the cloud catalog from loading, which has to start on its own inside the same `listModels` call. A desktop preferences file whose contents cannot be parsed is quarantined as `app-preferences.json.corrupt-<timestamp>` and the app continues with defaults, while operational read errors such as permission or sharing failures are reported instead of moving the file aside. One broken file thus costs the quarantined preferences instead of permanently hiding every model.
...
Local proxy configuration explicitly distinguishes three modes: `system` follows the system proxy, `direct` forces a direct connection, and `custom` uses the proxy URL stored by Wework. Existing configurations with a saved URL are read as `custom`; devices without a configuration default to `system`. Direct mode does not call Electron's system proxy resolver and passes an empty proxy to the Codex runtime.

System mode resolves the system proxy, including PAC, through Electron using the actual target URL, without caching a global proxy address. For example, PAC may route ChatGPT through a proxy while connecting directly to the internal Wegent model gateway; the ChatGPT result must not be reused for internal requests. Task creation, follow-ups, and model switches resolve the proxy again. The settings page displays the saved mode separately from the effective connection; when the system does not use a proxy for the current address, it shows "Direct (system proxy not used)." Cloud execution requests do not use the desktop's PAC.

Codex Responses-compatible models may be routed through the executor's built-in `codex responses proxy`. Cloud model gateways, local custom models, and standalone harnesses select proxies for their own request URLs; custom providers in Codex `config.toml` obtain their URLs through `config/read`. Vision sidecars resolve their own destinations independently, and an explicit `DIRECT` must not inherit the primary model's proxy. Official Codex account connections still configure their process proxy at startup using the ChatGPT URL, so changing that proxy still requires a Codex restart. Logs must not expose proxy credentials.
