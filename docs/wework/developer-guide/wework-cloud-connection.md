...
DeepSeek V4-Flash is a built-in provider profile. Its upstream endpoint is `https://api.deepseek.com/responses`, its catalog ID is `wework-deepseek-v4-flash`, and its context window is 1,048,576 tokens. It supports `low`, `high`, and `max` reasoning effort with `high` as the default. The catalog enables parallel tools, multi-agent v2, and Web Search, while declaring text-only input and no image generation. After Wework discovers models from DeepSeek `/models`, it keeps only `deepseek-v4-flash`, which is currently supported by the Codex profile. Existing default Chat Completions configurations managed by this profile are migrated on read to the Responses API, the `custom` tool profile, and live search.
