...
A local proxy entered in Wework takes precedence over the system proxy. Otherwise, local model execution requests resolve the system proxy, including PAC, through Electron using the actual target URL, without caching a global proxy address. For example, PAC may route ChatGPT through a proxy while connecting directly to the internal Wegent model gateway; the ChatGPT result must not be reused for internal requests. Task creation, follow-ups, and model switches resolve the proxy again. Cloud execution requests do not use the desktop's PAC.
