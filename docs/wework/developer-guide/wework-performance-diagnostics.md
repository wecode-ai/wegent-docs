...
A spinner should keep its SVG static inside a fixed-size HTML wrapper and animate the wrapper's `transform` with `will-change: transform`. A text shimmer should keep the base text normally rendered and sweep a fixed-width clipping window over one highlighted text copy. The window and copy use synchronized, opposing `transform` animations so the highlighted glyphs remain stationary. Do not split a persistent animation by grapheme because its animation nodes and `animationiteration` events grow linearly with text length. A gradient mask may soften the sweep edge, but the mask position must remain static. Both implementations must preserve `prefers-reduced-motion` behavior.
