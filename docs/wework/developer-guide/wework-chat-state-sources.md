...
| Guidance messages                     | One source per lifecycle phase: `queuedMessages` while pending, `messages` after application in the mounted pane, and the Codex transcript after reopening | `ConversationQueuePanel`, `MessageList`                                                                            | Do not store the same guidance simultaneously in the queue, background message cache, and Provider transcript; a background applied event only settles the queue, and reopening reads only the Codex transcript                             |
...
If the source pane is unmounted when guidance is applied, the background subscriber may only remove the matching item from `queuedMessages`; it must not append that item to `runtimeConversationCache.messages`. When the user reopens the source conversation, the native user message and its position come from the Codex transcript, so the cache never becomes a second transcript source.
