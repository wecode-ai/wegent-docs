...
Cloud installation records define the expected state. Reconciliation restores missing plugin contents and runtime registrations at the device's selected version and removes managed remnants of cloud-uninstalled plugins. Installed copies shared directly with you are included. Source files for plugins you created or imported, independent Skills, and MCP servers are not replaced or removed. If a cloud installation record still exists, refresh restores the installation; use the plugin's **Uninstall** action to remove it.
...
Choose **Create Smart app** from **My creations**, enter a name and save location, and Wework creates a valid DSH Web-preset directory, links it directly into the local list, and opens a new chat with the bundled Smart App Builder plugin. Creation and continued development use the local device and directory and do not require a cloud connection. A dedicated development-preview tab opens in the right workspace by default. Its single toolbar row shows the workbench name, runtime status, verification result, **Verify**, **Add plugins**, **Refresh page**, and **Reload DSH**. A failed verification shows the first blocking error code, file, and repair hint, and can expand to every issue; source changes mark the previous verification as stale. Startup and reload use a DSH-specific progress view instead of the regular built-in browser's empty URL state. Refresh the current page after frontend-only changes, or reload the DSH runtime after dependency or Harness configuration changes.
...
Choose **Link folder** to use an existing source directory without packaging it first. When distribution is needed, **Export installation package** requires the current source to pass verification, then rechecks the ZIP against its content fingerprint before producing a deterministic package; stale or failed verification cannot be exported. **Import app** remains available for existing ZIP files, and the managed imported directory can also be developed further. Marketplace or shared installations cannot be edited in place; choose **Copy to My workbenches** and assign the copy a new name and directory. Removing a linked workbench unregisters it without deleting its external folder. Use **Publish** to add marketplace details and select members or departments; marketplace downloads still use the native preview to confirm version compatibility and bind a model.
...
The Sites list contains both Sites you own and Sites where you are a collaborator. A Site owner can use **… → Manage collaborators** to add or remove collaborators by username. Collaborators can continue development, edit, publish, delete, configure environment variables, and update internal Site access, but cannot manage collaborators. For an internal Site, use **… → Access permissions** to allow anonymous users, signed-in users, project members only, or project members plus specified users; project owners and collaborators always retain access. Clicking a Site URL in the list opens it in the system browser. **Continue developing** makes sure `wegent-sites` is installed on the selected device and inserts both the plugin reference and Site reference into a fresh task composer so the AI receives the Site workflow and target-project context together.
