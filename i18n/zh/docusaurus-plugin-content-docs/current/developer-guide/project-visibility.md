...
“仅看相关任务”只适用于内置任务来源。它允许所有登录用户发现并进入项目，但不会向普通访问者暴露完整 Issue 集合。Owner 和 Maintainer 可以读取全部 Issue；其他用户只可读取自己创建、被指派、作为协作者参与、存在有效任务关联，或指派给自己所创建机器人的 Issue。列表、详情、交付物、执行记录、项目会话和“我的工作”入口复用同一套相关性判断；读取无关 Issue 时返回未找到，避免泄露其存在。

```mermaid
flowchart LR
    A[公开或仅看相关任务的项目] --> D[合并项目 ID 与有效角色]
    B[项目成员记录] --> D
    C[空间成员记录] --> E[空间项目关联]
    E --> D
    D --> F[项目列表与详情鉴权]
...
本次实现不修改数据库表结构和索引，也不引入授权缓存。访问范围仍位于 JSON 中，公开项目和“仅看相关任务”项目查询的实际扫描成本需在 MySQL 上通过执行计划验证。列表沿用现有全量返回协议，尚未引入导航分页；查询次数固定不代表返回数据量固定。
