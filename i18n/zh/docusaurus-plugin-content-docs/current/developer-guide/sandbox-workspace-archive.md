...
| --- | --- | --- | --- |
| `executor` | `$HOME` 中的 Claude 配置、`/workspace/{task_id}` | `home/`、`workspace/` | 默认值，只保留 `.claude/` 与 `.claude.json`，避免归档凭证和运行时挂载 |
...
executor 的 home 恢复同样使用 allowlist。旧归档中如果包含 `.ssh/`、`.npmrc`、`.config/`、`.local/`、`.gvm/` 或 code-server 运行时目录，envd 会跳过这些成员，避免恢复凭证或写入 Kubernetes 只读挂载。

## 旧归档兼容

早期 executor 归档没有 `home/` 与 `workspace/` 根目录：工作区文件直接位于 tar 根目录，Claude home 配置位于 `__home__/.claude/` 与 `__home__/.claude.json`。当前恢复逻辑会兼容该格式：

- tar 根目录下不属于 `home/`、`workspace/`、`__home__/` 的成员按旧工作区内容恢复到 `/workspace/{task_id}`。
- `__home__/.claude/` 与 `__home__/.claude.json` 按 executor home allowlist 恢复到运行时 home。
- 恢复仍会应用排除规则，跳过缓存、大目录和 code-server 运行状态。
- 恢复日志会输出新旧格式分别恢复的成员数量，便于排查“下载成功但内容为空”的情况。
