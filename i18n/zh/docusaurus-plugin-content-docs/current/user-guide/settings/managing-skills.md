...
技能是可复用的能力包。上传或导入技能后,它会先进入技能库；随后可以设为“默认启用技能”、成为“智能体自带能力”,或在聊天里“本次临时使用”。
...
**Skill** 是一个能力扩展包,包含可执行代码、配置和文档。当任务启动时,Skills 会被部署到 `~/.claude/skills/` 目录,扩展智能体的能力。
...
技能库解决“我有哪些技能”,使用位置解决“这些技能在哪里会被用”。

| 使用位置       | 含义                   | 何时生效                                                     |
| -------------- | ---------------------- | ------------------------------------------------------------ |
| 默认启用技能   | 跟着当前用户走         | 使用默认助手、系统助手、共享助手或自己创建的智能体时自动可用 |
| 智能体自带能力 | 跟着智能体走           | 选择该智能体后自动可用,不能在聊天里临时移除                  |
| 本次临时使用   | 只跟着当前聊天或任务走 | 在输入框技能按钮里临时勾选后生效                             |

关键规则:

- 新上传的个人技能会进入技能库,并自动设为上传者的默认启用技能。
- 新上传到组里的技能属于组,但只会对上传者自动默认启用。
- 组技能设为默认启用只影响你自己,不会影响其他组员。
- 系统技能和资源库技能不会自动默认启用,可以手动设为默认启用。
- 存量技能不会因为升级而自动默认启用。
...
- **默认启用技能**: 展示会自动跟随你进入对话的技能,可以直接取消默认启用。
- **技能库**: 展示你能使用和管理的所有技能,可以把技能设为默认启用。

点击 **添加默认启用技能** 时,系统会从你可用的所有技能中选择,不需要先切换到个人或团队来源。个人、团队、系统和资源库只是技能来源,不是默认启用的归属。
...
- 描述(前 2 行)
- 版本、作者、标签
- 技能来源:我的技能、团队技能、系统技能或资源库技能
- 默认启用状态
- 最后更新时间

资源库页面先选择资源类型(例如技能、智能体、模型),再在类型内部按来源筛选。来源筛选默认是“全部”；选择“团队”时可以查看全部团队,也可以在下拉列表中选择单个团队。来源筛选只决定列表里显示哪些资源,不会改变“默认启用技能”属于当前用户这一点。团队资源会在列表项里显示所属团队,便于区分来源。

### 管理默认启用技能

在技能库中点击:

- **设为默认启用**: 之后你在默认助手、系统助手、共享助手和自己创建的智能体中都可以自动使用这个技能。
- **取消默认启用**: 之后该技能不再跟着你自动生效,但技能资产仍保留在技能库中。

这个操作只影响当前用户,不会修改智能体、系统助手或组里的技能资产。

在 **高级设置** 中,可以继续为每个默认启用技能配置:

- **启用的模式**: 默认全部启用,取消勾选的模式不会自动启用该技能。
- **启用的智能体**: 默认全部启用,可以按“我的智能体”“团队智能体”“系统智能体”整体取消,也可以取消单个智能体。
- **强制激活**: 开启后,该技能会直接进入任务上下文,走 preload 逻辑；关闭时,技能仍是自动可用,但由模型按需加载。
...
3. **保存智能体**
   - 点击"保存"按钮
   - Skills 现在成为该智能体的自带能力

如果你只是想让某个技能对自己所有对话自动可用,优先在技能库中使用“设为默认启用”,不需要为此复制系统智能体。
