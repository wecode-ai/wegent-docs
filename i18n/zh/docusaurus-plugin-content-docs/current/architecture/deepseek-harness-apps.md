...
flowchart LR
    ...
    RUNTIME[Wework 管理的单文件 DSH Runtime 归档] --> EXTRACT[(按内容指纹解包)]
    EXTRACT --> INSTANCE
...
| Runtime 归档、内容指纹和构建准备      | `wework/scripts/prepare-deepseek-harness-runtime.mjs`、`wework/src-tauri/tauri.conf.json`                   |
