...
在发生问题的任务右上角（或新对话状态的工具栏）点击 **反馈问题**，选择需要包含的信息，然后点击 **预览导出内容**。确认无误后点击 **确认导出**，Wework 会把诊断包保存到系统下载目录。

诊断包分为两组：

- **常规诊断**（默认勾选）：Wework 和本地执行服务的运行日志、Wework 版本和系统信息。不含对话内容，隐私风险低。
- **完整任务数据**（默认不勾选）：完整对话记录原文和当前窗口截图。可能包含业务讨论、代码或屏幕上显示的任何内容，确认后再提供。

如果勾选的某项内容不存在（例如新对话没有任务数据），导出时会自动跳过该项，并在预览中提示。

导出前可以展开任意条目查看内容。诊断包只在本机生成，不会自动上传。日志中的常见凭证和用户目录会被脱敏，但自由文本（如对话记录）无法完全脱敏，发送前应确认包内没有不希望共享的信息。
