...
Wegent 云端市场会预先提供 `wegent-sites`，但不会提前安装到每个用户。打开 **站点** 并点击 **创建** 时，Wework 会为当前账号幂等安装该插件，将插件引用写入新任务输入框，并同步到该账号的在线本地设备和云设备。重复点击不会创建重复安装记录。
...
本地模式需要先连接 Wegent 云端。点击输入框中的 **站点** 插件标签会打开 Wegent 云端市场的插件详情页。
