...
从顶部标签栏的 **+ → 智能工作台** 可直接进入 **应用 → 智能工作台**；也可以先打开 **应用**，再从 **站点 / 小程序 / 智能工作台** 中切换。市场页和 **已安装** 是智能工作台类型下的两个分区。市场页的 **创建智能工作台** 会打开新对话并引用随 Wework 安装的“智能工作台开发助手”插件；该插件负责检查 DSH 开发环境、检索并组合 DSH 插件、在 Wework 内置浏览器中验证本地 profile，以及校验和打包安装 ZIP。安装仍必须回到 Wework 原生预览界面确认版本依赖和绑定模型，插件不得直接修改本机安装注册表。
