...
在 **我的创建** 点击 **创建智能工作台**，填写名称和保存位置后，Wework 会基于 DSH Web 预设生成一份合法的工作台目录，将它直接关联到本机列表，并打开引用“智能工作台开发助手”的新对话。右侧工作区会默认打开独立的开发预览标签页，在同一行显示工作台名称、运行状态、**添加插件**、**刷新页面** 和 **重新加载 DSH**。启动或重新加载期间只显示 DSH 专用进度，不会出现普通内置浏览器的 URL 空状态；文件改动后可以刷新当前页面，依赖或 DSH 配置变化后可以手动重新加载整个运行时。
