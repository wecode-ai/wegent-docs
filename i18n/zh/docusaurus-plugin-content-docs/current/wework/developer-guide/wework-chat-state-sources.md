...
| 引导消息                      | 按生命周期阶段唯一：待应用时为 `queuedMessages`；当前 pane 应用后为 `messages`；重新打开后为 Codex transcript | `ConversationQueuePanel`、`MessageList`                                                                | 同一时刻不能把引导同时写入队列、后台消息缓存和 Provider transcript；后台应用事件只结算队列，重新打开时只读取 Codex transcript                                                   |
...
如果引导应用时源 pane 没有挂载，后台订阅者只能从 `queuedMessages` 移除对应条目，不能把它追加到 `runtimeConversationCache.messages`。用户重新打开源对话时，由 Codex transcript 中的原生 user message 恢复内容和顺序；缓存不能作为第二份 transcript 信源。
