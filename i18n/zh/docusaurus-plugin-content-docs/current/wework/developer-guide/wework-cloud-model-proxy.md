...
通用云端模型可以复用 Wework 的 GPT 5.6 兼容目录作为能力基线，并由云端模型元数据覆盖上下文窗口。兼容目录不得通过 `max_context_window` 固定限制云端模型的有效窗口；该字段保持为空，由运行时采用云端下发的窗口配置。Codex 官方 GPT 5.6 目录仍保留自身明确的最大窗口限制。
