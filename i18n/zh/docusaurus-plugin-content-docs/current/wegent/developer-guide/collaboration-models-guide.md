...
Claude Code 和 Codex 执行器都将第一个机器人作为协调者，其余成员作为原生子代理。
Codex 为每个成员注册独立角色，使用该成员的提示词、模型及推理配置；未设置模型的成员继承协调者。
使用 API 模型时，各成员的供应商地址和凭据通过执行器代理独立路由，凭据不会写入子代理角色文件。
团队成员的 Skill 和 MCP 在同一执行环境中共享。Codex 配置或角色文件部署失败会使任务明确失败。

使用 Codex 官方登录时，成员可以继承登录并选择其他模型；若成员使用独立 API 供应商，协调者也需要配置 API 模型。
