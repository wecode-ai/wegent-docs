...
流式正文和滚动跟随各自只能有一个动画控制源，不能同时叠加组件动画、直接滚动和延迟滚动定时器：

- `useBufferedStreamingText` 是正文揭示节奏的唯一控制源。它按待显示字符数自适应加速，保留小数揭示余量，并按 Unicode code point 推进，不能再给 `Streamdown` 开启独立的逐字动画。
- `Streamdown` 继续使用 `streaming` 模式做增量 Markdown 解析；非尾部 Markdown 块继续使用现有窗口化和冻结逻辑，不能因动画重建整棵消息树。
- 屏外流式块暂停正文揭示；所有活跃流式块共享一个 FPS 采样循环。帧率降级时暂停新的正文提交，帧率恢复后继续消费同一队列，不能为每个块创建独立采样器。
- `ScrollableMessageArea` 的流式自动跟随只由一个阻尼弹簧驱动。ResizeObserver、消息追加和运行态变化只能请求这个弹簧继续追踪最新底部，不能同时调用流式直跳或稳定滚动定时器。
- 弹簧落后于布局增长时，通过共享的 reveal scale 对正文队列施加背压；背压只调节正文速度，不直接修改 Markdown、消息状态或虚拟列表测量。
- 用户主动向上滚动、turn navigation 暂停或自动滚动挂起时，立即取消弹簧并恢复 reveal scale。非流式布局稳定、历史恢复和用户点击“回到底部”仍使用各自原有路径，不与流式弹簧并发。
- `prefers-reduced-motion` 下跳过正文渐进揭示和弹簧运动，直接显示权威文本并定位到底部。
...
流式渲染改动还必须覆盖自适应揭示、Unicode 边界、布局背压、弹簧收敛、用户上滚暂停和回到底部恢复。真实 Electron 场景需要保留用户上滚前后的锚点截图，并确认后续流式追加不会移动该锚点。
