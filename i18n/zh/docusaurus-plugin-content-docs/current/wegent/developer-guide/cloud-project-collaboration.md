...
调度员完成拆分和分派不代表 Issue 已完成。父任务步骤和子任务详情会显示实际子任务的等待状态；缺少模型或工作区时，执行保持 `waiting_runtime`，通过「配置并继续执行」为原执行记录补齐配置。没有符合授权和容量条件的设备时，Run 保持排队并等待设备领取，而不是要求用户回到智能体中绑定机器。补齐运行配置不会修改项目或智能体默认配置，并保留人工审批要求。步骤通过验收后才计入工作流完成进度。
