...
云项目在「项目设置 → 分配与调度」配置当前用户的默认设备和模型，供 AI 调度员使用。自定义 Codex 智能体在创建时必须选择设备和模型；智能体列表中的「配置设备和模型」用于设置后续任务的默认执行配置。模型身份与提供方参数作为不透明字典传递，不参与接口字段的大小写转换。设备在线状态以连接心跳为准。AI 调度工作流缺少必要执行配置时，创建 Issue 返回明确的配置错误，且不会保存任务或启动调度；补齐配置后再创建。

调度员完成拆分和分派不代表 Issue 已完成。父任务步骤和子任务详情会显示实际子任务的等待状态；缺少设备或模型时，执行保持 `waiting_runtime`，通过「配置并继续执行」为原执行记录选择完整配置。该操作不会修改项目或智能体默认配置，并保留人工审批要求。步骤通过验收后才计入工作流完成进度。
