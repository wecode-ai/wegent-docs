...
Wework 桌面版在 Electron 运行时提供本地键盘快捷键。快捷键配置只保存在当前设备，不写入 Backend，也不会随云端连接同步。

## 打开设置

在 Electron 桌面版中，进入 **设置 → 个人 → 键盘快捷键** 可以查看和修改本机快捷键。每一行显示命令名称、说明、当前按键绑定，以及恢复默认或清除绑定的操作。
...
| 选择模型     | `Control+Shift+M`           | 打开或关闭当前输入区的模型选择器               |
