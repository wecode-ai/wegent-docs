...
队列按从上到下的顺序串行发送。Wework 发送队首消息后，会等待该消息对应的新 turn 确认启动；在此之前不会发送下一条，避免多个待发送消息并发提交。当前 turn 完成后，队列才会继续发送下一条消息。队列中有多条消息时，可以拖动每条消息左侧的抓手实时调整顺序。停止当前回复会同时暂停队列，不会立即发送下一条消息。点击“继续发送”会恢复引导状态，并直接发送队首消息。
...
侧边栏临时对话支持自动排队、取消和编辑：当前回复执行时提交的新消息会显示在待发送队列中，当前回复完成后自动发送。排队期间不会将“任务正在运行”作为发送失败展示；可以在队列卡片中取消或编辑待发送消息。
