...
在 Wework 桌面端，可以直接复用右侧工作区和底部终端中的上下文：

- 将右侧文件树中的文件或文件夹拖入输入框，会以工作区路径引用的形式加入当前会话。
- 在代码或 Markdown 文件预览、文件编辑器或终端中选中文本后，选区旁会显示“添加到对话”浮层；点击即可把文本加入当前输入框。
- 也可以直接拖动已选文本并放入输入框。开始拖动后，桌面会显示系统拖拽面板；结束或取消拖动后面板会自动关闭。
