...
对话中的文件变更列表还会显示每个文件的实际编辑耗时。该时间从对应编辑工具开始调用起，计算到工具流式输出和执行全部结束；同一文件被多次编辑时，覆盖首次编辑开始到最后一次编辑结束。这样，任务末尾生成变更摘要的时间不会再被误显示为文件编辑耗时。

二进制文件没有可靠的文本行数，因此只显示文件名称和“二进制文件”状态。

对于整文件新增或删除事件，Executor 会先把完整文件内容规范化为 unified diff，再交给变更摘要、悬浮预览和审核界面使用。因此，新增文件的每一行都带有新增标记；Markdown 列表自身的 `-` 仍属于文件内容，不会被误判为删除行。
